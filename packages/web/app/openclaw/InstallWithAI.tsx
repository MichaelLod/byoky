...
- The Byoky bridge is a tiny local HTTP proxy on 127.0.0.1:19280. OpenClaw talks to the bridge, which forwards through Chrome native messaging to the extension, which calls the provider.
...
1. Install the bridge globally and register the native messaging host:
     npm install -g @byoky/bridge
     byoky-bridge install

2. Install the OpenClaw plugin globally (registers all 13 Byoky providers with OpenClaw):
     npm install -g @byoky/openclaw-plugin

3. Authenticate OpenClaw against a Byoky provider. Default to Anthropic; ask me first if I want a different one (byoky-openai, byoky-gemini, byoky-xai, byoky-deepseek, byoky-mistral, byoky-groq, byoky-cohere, byoky-perplexity, byoky-together, byoky-fireworks, byoky-openrouter, byoky-azure_openai):
     openclaw models auth login --provider byoky-anthropic
   This opens my browser so my wallet can approve the connection. Wait for me to confirm I approved it.

4. Verify the bridge is up:
     curl http://127.0.0.1:19280/health
   Expected: {"status":"ok","providers":["anthropic"]}

5. Inside OpenClaw I can run /byoky to see bridge status and connected providers.
