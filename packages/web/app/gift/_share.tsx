...
export const ogAlt = 'Byoky token gift';
...
function formatExpiryLong(ms: number): string {
  const diff = ms - Date.now();
  if (diff <= 0) return 'Expired';
  const hours = Math.floor(diff / 3_600_000);
...
export async function resolveGiftShortId(shortId: string): Promise<string | null> {
  try {
    const res = await fetch(`${VAULT_URL}/gift-links/${encodeURIComponent(shortId)}`, {
      cache: 'no-store',
    });
    if (!res.ok) return null;
    const data = (await res.json()) as { encoded?: string };
    return typeof data.encoded === 'string' ? data.encoded : null;
  } catch {
    return null;
  }
}

export function buildGiftMetadata(encoded: string | null, canonicalUrl: string): Metadata {
  const link = encoded ? decodeGiftLink(encoded) : null;
  ...
  const description = link && valid
    ? `${link.s} is sharing ${formatTokens(link.m)} ${link.n} tokens with you via Byoky · ${formatExpiryShort(link.e)}`
...
export function renderGiftOgImage(encoded: string | null): ImageResponse {
  const link = encoded ? decodeGiftLink(encoded) : null;
  const valid = link && validateGiftLink(link).valid;
      ...
      >
        ...
        >
          {gift ? (
            ...
            >
              <div style={{ display: 'flex' }}>{formatExpiryLong(gift.e)}</div>
              <div
                style={{
                  width: 6,
                  height: 6,
