...
export function PayDemo() {
  ...
  // Initialize SDK
  useEffect(() => {
    // appId, vaultUrl, walletUrl are added in the billing PR — cast for now
  ...
  async function connectWallet() {
    ...
    try {
      ...
      try {
        const bal = await (byokyRef.current as any).getBalance?.();
  ...
  async function sendMessage() {
    ...
    try {
      ...
      // Refresh balance
      try {
        const usage = await session.getUsage() as any;
